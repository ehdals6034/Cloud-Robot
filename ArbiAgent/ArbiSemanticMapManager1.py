...
class MapManagerDataSource(DataSource):
    def __init__(self, broker_url):
        ...
        #LTM으로부터 정보를 받고싶을 때 subscribe(subscribe를 걸면 notify를 받을 때 on_notify가 켜짐)
        #rule 뒤 형식으로 올라오면, notify 뒤 형식으로 정보 받음
    ...
    #notify를 받을때 켜짐 
    #subscribe한 3개에 대해 (Robotinfo, DoorStatus, MosPersonCall) 어떤 것을 notify 할지 선언
    def on_notify(self, content): # executed when the agent gets notification from ltm
        # print("[on Notify]" + content)
        time.sleep(0.05)
        gl_notify = GLFactory.new_gl_from_gl_string(content) #gl을 string으로 가져옴

        #SMM update에 사용했던 정보들로 바꾸어줌(id, pos, load)
        #gl의 name(첫 번째 자리)에 'RobotInfo'가 들어왔을 때
        if gl_notify.get_name() == "RobotInfo": # "RobotInfo" notification from ltm
            ...
            temp_RobotInfo.pos = [gl_notify.get_expression(1).as_value().float_value(), #position x,y좌표
        ...
        #gl의 name(첫 번째 자리)에 'DoorStatus'가 들어왔을 때
        ...
        #gl의 name(첫 번째 자리)에 'MosPersonCall'가 들어왔을 때
...
class MapManagerAgent(ArbiAgent):
    def __init__(self):
        ...
        self.robot_goal = { #SMM도 자체적으로 Robot의 goal을 가지고 있음
            "AMR_LIFT1": -1,
            "AMR_LIFT2": -1,
            "AMR_TOW1": -1,
            "AMR_TOW2": -1
    ...
    #Cargo에 대한 정보를 gl 형식으로 만들어서 CM에게 notify해줌
    ...
    #Rack에 대한 정보를 gl 형식으로 만들어서 CM에게 notify해줌
        ...
        while True:
            ...
            for id in temp_RackPose_LIFT_info.keys():
                ...
                if id != -1: # check rack is moving
                    ...
                    self.notify(consumer, gl) #TM에게 정보 assert
                else:
                    pass
    ...
    #4개의 robot에 대한 정보를 NC한테 한 번에 보내줌
    def MultiRobotPose_notify(self, consumer): # Notify MultiRobotPose every 1 sec to NC
        ...
        while True:
            ...
            self.notify(consumer, MultiRobotoPose_gl) #nc에게 notify

    #각 로봇에 대한 정보(LIFT, TOW)를 CM에게 보내줌
    ...
    #위에서 thread로 켜줌(주기적으로 도는 함수)
    def Plan_update(self, robot_ids):
        while True:
            time.sleep(0.5)
            for robot_id in robot_ids:
                # 로봇이 지나간 path를 삭제하는 함수
                self.ltm.MC.update_NAV_PLAN(robot_id)

    #SMM 내부에서 주기적으로 collision check
    def Collidable_notify(self, consumer): # Check Collidable every 2 sec and Notify if it has to NC
        ...
        while True:
            ...
            #collision이 있으면 gl 형태로 만들어서 notify 
    ...
    #물건을 싣기전 path만 nc에게 받음
    #큰 단위의 goal을 TA에게 알려줌
    def on_data(self, sender, data):
        gl_data = GLFactory.new_gl_from_gl_string(data)
        data_name = gl_data.get_name()
        
        if data_name == "GoalAssigned": #goal일 할당되면 
            robot_id = gl_data.get_expression(0).as_value().string_value()
            goal_id = gl_data.get_expression(1).as_value().string_value()
            self.robot_goal_id[robot_id] = goal_id  #goal id 입력
        elif data_name == "GoalCompleted": #goal이 끝나면
            robot_id = gl_data.get_expression(0).as_value().string_value()
            self.robot_goal[robot_id] = "0" #goal id에 '0' 입력 #robot_goal_id

    #parameter T가 빠짐(수정 필요)
    #detect collision을 계산해서 물어보는 애한테 넘겨줌
    def on_query(self, sender, query): # executed when gets query
        ...
        if query_name == "Collidable":  # From Local CM
            ...
            temp_Collidable_info = self.ltm.MC.detect_collision() #T가 빠짐
            temp_Collidable_num = len(temp_Collidable_info)
            temp_Collidable_block = " (pair \"{robot_id1}\" \"{robot_id2}\" {time})"
        ...
        #어디를 향해 가고있는지, 큰 단위의 goal을 가지고 있는지(goal vertex, goal ID) TA에게 알려줌
        elif query_name == "RobotSpecInfo":  # From Local TA
            ''' RobotSpecInfo gl format: (RobotSpecInfo (RobotInfo $robot_id (vertex_id $v_id1 $v_id2) $load $goal $goalID), …) '''
            ...
            for i in range(temp_robot_num):
                ...
                if "TOW" in temp_robot_id:
                    ...
                    elif not temp_TOW_path:  #path가 없으면 '0' 대입
                        temp_TOW_goal = 0 #아래 else문 추가
                    temp_TOW_goal_id = self.robot_goal_id[temp_robot_id]
