...
class MapManagerDataSource(DataSource):
    def __init__(self, broker_url):
        ...
        #LTM으로부터 정보를 받고싶을 때 subscribe
        #rule뒤에 있는 형식으로 올라오면 notify뒤 내용 형식으로 한다
...
class MapManagerAgent(ArbiAgent):
    ...
    def Collidable_notify(self, consumer): # Check Collidable every 2 sec and Notify if it has to NC
        ...
        while True:
            ...
            if temp_Collidable_num:
                ...
                #colision이 있으면 위와같이 gl을 만들어서 notify해줌
    ...
    def on_data(self, sender, data):
        ...
        elif data_name == "GoalCompleted": #goal이 끝났다
            robot_id = gl_data.get_expression(0).as_value().string_value()
            self.robot_goal[robot_id] = "0" #id를 0으로 
    ...
    def on_query(self, sender, query): # executed when gets query
        ...
        if query_name == "Collidable":  # From Local CM #아직 사용 안하고있음
            ''' Collidable gl format: (Collidable $num (pair $robot_id $robot_id $time), …) '''
            ...
            #robot id와 어디에있는지 goal이 있는지 없는지
