...
class NavigationControllerAgent(ArbiAgent):
    def __init__(self):
        ...
        self.move_flag = { #안써도 됨
            "AMR_LIFT1": False,
            "AMR_LIFT2": False,
            "AMR_TOW1": False,
            "AMR_TOW2": False
        }  # identify whether robot is moving
        self.avoid_flag = { #켯을때 colision들어온거 무시
            "AMR_LIFT1": False,
            "AMR_LIFT2": False,
            "AMR_TOW1": False,
            "AMR_TOW2": False
    ...
    def on_start(self):  # executed when the agent initializes
        ...
        self.main_loop()
    #on_start와의 관계? #clear_generator_queue와 함께 check
    def main_loop(self):
        cleanse_list_gen = self.cleanse_list() ##왜사용?
        while True:
            clean_generator_queue = []
            for task in self.generator_queue:  #어떤형태?for
                try:
            ...
            self.generator_queue = clean_generator_queue  #generator queue 생성
    ...
    #notification이 multirobotpose/Colliable로 들어온다
    ...
    def update_collidable(self, temp_gl):
        ...
        for i in range(collide_num): #들어온 충돌횟수만큼 일반적으로 1
    ...
    #SMM에게 로봇 위치(Vertex)를 받으면 로봇 path전달
    def update_multi_robot_pose(self, temp_gl):
        ...
        for i in range(robot_num):  # update robot pose from gl
            ...
            robot_pose_gl = temp_gl.get_expression(i).as_generalized_list()  # get ith gl content #이렇게 gl 객체로
            ...
            robot_vertex = [robot_vertex_gl.get_expression(0).as_value().int_value(), #vertex는 integer타입
                            robot_vertex_gl.get_expression(1).as_value().int_value()]  # current pose to list
            multi_robot_pose[robot_id] = robot_vertex     #update multi robot pose
            self.cur_robot_pose[robot_id] = robot_vertex  # update current robot pose in agent 여기도 추가
        ...
        if robot_id_BI:  # check whether robot_id_BI is empty
            for robot_id in robot_id_BI: #path가 새로 생긴 로봇에 대해서
                #SMM이 새로운 path를 알 수 있도록 알림
                self.semantic_map_manager_notify(robot_id)  ## notify SMM of same control information
                # if (not self.avoid_flag[robot_id]) and (not self.move_flag[robot_id]):
                #회피하고 있는 경우(크기가 2이상?)는 새로운 명령을 안받도록
                if not self.avoid_flag[robot_id]: 
                    #1.지금하는거 cancel 2.새로운 path를 실행하도록 
                    print("[INFO] {RobotID} cancel move".format(RobotID=robot_id))
                    self.generator_queue.append(RobotTask(self.cancel_move(robot_id), "cancel_move"))
                    print("[INFO] {RobotID} Control request by <COLLIDABLE> Notification [{num}]".format(
                        RobotID=robot_id, num=self.count))
                    ...
                    self.count = self.count + 1 #몇번째로 control_request가 사용되었는지 지금은 무시
    
    #Robot TM으로부터 nc가 goal을 받음
    def on_request(self, sender, request):  # executed when the agent gets request
        ...
        if temp_gl.get_name() == "Move":  # "Move" request from robotTM
            ...
            requested_robot_id = robot_id #젤 아래 Robot_id에 집어넣기 위해서
            self.goal_actionID[robot_id] = action_id  # update actionID in agent #action이 끝났다고 알려주기 위해서
            temp_start = temp_gl.get_expression(2).as_value().int_value() #get startVertex
            real_goal = temp_gl.get_expression(3).as_value().int_value()  # get goalVertex
            self.real_goal[robot_id] = real_goal #nc가 알아야하는 goal
           ...
           #call by reference가 아닌 call by value로 가져옴
            ...
            ''' robot_id_replan: robot which needs to get replanning -> MAPF
                robot_id_BI: robotID that has any change of its path and notify the information of robotID to robotBI ''' #기존에 path를 가진 로봇에 대해서
                #goal + currnet command를 가지는 로봇
            #check
            #계속 진행을 하는지(현재 path까지 진행) or 멈추는건지
            if robot_id_BI:  # check whether robot_id_BI is empty
                for robot_id in robot_id_BI:
                    if len(robot_id_replan) < 2: #1이다? #cancelmove #control하는 거 켜줌 => path가 1이면 회피하고 있는 경우가 아니라서
                        self.semantic_map_manager_notify(robot_id)  ## notify SMM of same control information
                        print(
                            "[{action_id}][INFO2] {RobotID} cancel move".format(action_id=action_id, RobotID=robot_id))
                        self.generator_queue.append(RobotTask(self.cancel_move(robot_id), "cancel_move")) #move를 cancel
                        ...
                        self.generator_queue.append(RobotTask(self.control_request(robot_id, False, True, self.count), robot_id))
                        self.count = self.count + 1

            #MAPF에게 물어보는 부분
            if robot_id_replan:  # check whether robot_id_replan is empty
                #1.움직이는 두 로봇을 cancel시킴
                ...
                #NC내부 업데이트
                robot_ids = self.multi_robot_path_update(path_response_gl)  # update path of robot in NC #robot_id를 가져옴
                #위에서 가져온 로봇2개에 대해서
                for robot_id in robot_ids:
                    self.semantic_map_manager_notify(robot_id) #update된 path를 mapmanager에게 알려줌
                #위에 로봇이 하나만 움직이는 경우에 대해서는 고려가 안되어있음

                #replan이 필요한 로봇에 대해서 #current_command가 비어있음
                for robot_id in robot_id_replan:
                    if self.navigation_controller.current_command[robot_id]:
                        ...
                        # Thread(target=self.Control_request, args=(robot_id, False, True, self.count,), daemon=True).start()  # control request of robotID
                        self.generator_queue.append(RobotTask(self.control_request(robot_id, False, True, self.count), robot_id)) #control할 수 있게 넣어줌
                        print("generator queue appended")
                    else: #move_flag는 신경x
                        self.move_flag[robot_id] = True
                    self.count = self.count + 1
            #TM에게 결과가 어떻든 request를 잘 받았다고 알려줌
            ...
            return goal_request_result #query나 request경우 resonse로 return
        #정보를 빼주고 guide move함수를 실행
        #로봇이 나올 때 근처에 countpart로봇이 있는지 check
            ...
            print("GGGG")
            self.generator_queue.append(RobotTask(self.guide_move(robot_id, action_id, vertex, direction), robot_id))
            print("GGGG")
            return "(ok)"
            ...
            vertex = temp_gl.get_expression(2).as_value().int_value()
            print("PPPP")
            self.generator_queue.append(RobotTask(self.precise_move(robot_id, action_id, vertex), robot_id))
            print("PPPP")
            return "(ok)"
            ...
            vertex = temp_gl.get_expression(2).as_value().int_value()
            print("SSSS")
            self.generator_queue.append(RobotTask(self.straight_back_move(robot_id, action_id, vertex), robot_id))
            print("SSSS")
            return "(ok)"
    ...
    def guide_move(self, robot_id, action_id, vertex, direction):
        ...
        while True:
            ...
            #counterpart 로봇의 앞 vertex3개에 current_command가 포함되어 있으면 멈춰라
    ...
    def control_request(self, robot_id, collide, replan, count):
        ...
        #current_command에 robot_id가 들어올때까지 계속 실행
        ...
        if self.navigation_controller.current_command[robot_id]: #path를 받았을 때
            ''' if robot is avoiding against counterpart robot, path of the robot is split
                e.g. self.NC.robotTM_set[avoidingRobotID] == [[path], [path]]
                     self.NC.robotTM_set[counterpartRobotID] == [[path]] '''
        ...
        #collide_flag를 끄는 부분 이부분 check
        #counterpart에 해당하는 로봇도 꺼줌
        if collide:
    ...
    def multi_path_control(self, c, robot_id):
        ...
        robot_index = self.AMR_IDs.index(robot_id)
        c_robot_id = self.AMR_IDs[counterpart_check[robot_index]]  # get robotID of counterpart robot
        self.avoid_flag[c_robot_id] = True #이것도 check
        ...
        for path_idx in range(len(currnet_command_set[robot_id])):  # consider path set
            ...
            if current_command_set_start_condition[robot_id]:
                if current_command_set_start_condition[robot_id][path_idx]:  # check whether current path has any start condition
                    wait_flag = True
                    #로봇이 scond를 만족하여 새로운 path를 받게되었을 떄 기다리는 부분 
                    while wait_flag:
                        print("[MULTI_PATH_CONTROL]yield waiting current_command_set_start_condition")
                        #잠시 넣어둔것 나중에 check
                        if self.real_goal[c_robot_id] == -1: #goal을 마쳤다 => executed idx가 바뀜
                            break
                        else:  #check
                            for cond in current_command_set_start_condition[robot_id][path_idx]:# [1,2],[3,4],[]
        ...
        yield from self.goal_check(robot_id, action_id) #multipathcontrol이 끝났으니 goal check
    ...
    def single_path_control(self, c, robot_id):
        ...
        while not self.navigation_controller.current_command[robot_id]:
            ...
            yield
        if self.navigation_controller.current_command[robot_id]: #왜 current_command를 가져오지 않았을까?
       ...
       #bi에게 보낼 path_gl을 만듦
        move_gl_str_format = "(move (actionID {actionID}) {path})"
        path_gl = self.path_gl_generator(robot_path, robot_id)  # convert path list to path gl
        #path_set에 있는 path_id를 랜덤하게 보냄
        action_id = "\"" + str(uuid.uuid4()) + "\"" #겹치는 문제가 있어서action id를 랜덤하게 바꿈
        move_gl = move_gl_str_format.format(actionID=action_id, path=path_gl)
        self.semantic_map_manager_notify(robot_id) #새로운 path를 mapmanager에게
        #BI에게 request하고 response를 받음
        print(
    ...
    def semantic_map_manager_notify(self, robot_id):  # notify SMM of control information
        ...
        if self.navigation_controller.current_command[robot_id]: #currnet_command가 비어있지 않은 경우에
            ...
            self.notify(self.SMM_name, SMM_gl)  # notify SMM #로봇 path 바뀐것을 SMM에게 알려줌
        else:
    ...
    def goal_check(self, robot_id,
        ...
        while True:
            ...
            #끝났는데 안끝났다 돼있을 경우
            if is_goal_terminated == -1: #안끝났다
                #로봇의 현재 pose와 goal이 같다.
                if self.navigation_controller.robotPose[robot_id][0] == self.navigation_controller.robotPose[robot_id][1] == self.navigation_controller.robotGoal[robot_id]: #현재 pose가 goal vertex와 같다면
    ...
    def multi_robot_path_update(self, response_gl):  # update paths of robots in NC
        ...
        robot_ids = []
        for i in range(robot_num): #로봇2개에 대해서
            ...
            print('robot ID : ' + str(robot_id)) #robot id
            print('path : ' + str(path)) #path
        self.navigation_controller.get_multipath_plan(multipaths)  # update path in NC #mutipath를 업데이트
        command_set = self.navigation_controller.command_set #command_set을 가져옴
        ##path의 개선점
        #path set의 길이가 2개 이상일 때 [123][3456]
        #앞 path의 마지막 vertex를 해당 path첫부분에 넣어준다[[1,1]]
        ...
        #위에서 get multipath plan을 하여서 update_robot_TM을 함
        self.navigation_controller.update_robot_TM(robot_pose)  # update pose in NC

        return robot_ids
    #MAPF에게
    #tow1,2 / lift1,2에대해 두봇에 대한 정보를 한번에 보내줌
    def multi_robot_path_query(self, replanned_robot_ids):  # query about path to MultiAgentPathFinder(MAPF)
        ...
        for robot_id in replanned_robot_ids:
            ...
            start_id = self.cur_robot_pose[robot_id][0] #현재로봇위치
            ...
            if goal_id == -1:  # check whether robot has no goal
                goal_id = start_id  # no goal -> start==goal #goal과 start id가 같다
            ...
            if len(replanned_robot_ids) == 1:  # check wheather robot_id_replan(argument) has one ID
                ''' if only has
                        ...
                        TOW1 -> add TOW2
                        TOW2 -> add TOW1 '''
                #counterpart robot에 해당하는 부분
                counterpart_check = {0: 1, 1: 0, 2: 3, 3: 2} #counterpart로봇이 뭔지 체크
                robot_index = self.AMR_IDs.index(robot_id)
                c_robot_id = self.AMR_IDs[counterpart_check[robot_index]]  # get robotID of counterpart robot
